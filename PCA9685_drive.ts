...
/**
 * Microbit_V1.1 扩展板 电机、舵机、板载LED 驱动程序，基于PCA9685。
 */
...
namespace PCA9685_drive {
	...
	*/
    //% block="Led|%index|Crt %onoff"  group="LED灯"
    export function Led(index: Leds, onoff: LEDCrt):void {
        if (!initialized) {
            initPCA9685()
        }
        if (onoff == LEDCrt.led_on) {
            switch (index) {
                case Leds.led1:
                    setPwm(6,0,4095);
                    break;
	 ...
	 * 板载LED控制，基于PCA9685扩展引脚
	*/
    //% block="All_LED_ON"  group="LED灯"
    export function ON_All_Led():void {
        if (!initialized) {
            initPCA9685()
        }
        setPwm(6,0,4095);
	 ...
	 * 板载LED控制，基于PCA9685扩展引脚
	*/
    //% block="All_LED_OFF"  group="LED灯"
    export function OFF_All_Led():void {
        if (!initialized) {
            initPCA9685()
        }
        setPwm(6,0,0);
